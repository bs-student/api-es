...
//                            'errorDescription' => "You can't contact for your own textbook deal."
                    'errorTitle' => "Imposible contactar",
                    'errorDescription' => "Imposible negociar contigo."
                ), 400);
            }
...
//                                $message="We have sent you ".$bookDeals[0]->getSeller()->getUsername()."'s contact information over your mail. Please contact ".$bookDeals[0]->getSeller()->getUsername();
                        $message = "Enviado por email la información de contacto de " . $bookDeals[0]->getSeller()->getUsername() . ". Por favor contacta con " . $bookDeals[0]->getSeller()->getUsername();
                        $buyerInfo = array(
...
//                                $message="We have sent ".$bookDeals[0]->getSeller()->getUsername()." your contact information. ".$bookDeals[0]->getSeller()->getUsername()." will contact you as soon as possible.";
                        $message = "Enviado por email la información de contacto de " . $bookDeals[0]->getSeller()->getUsername() . ". " . $bookDeals[0]->getSeller()->getUsername() . " te contactara lo antes posible.";
                        $buyerInfo = array(
                            'buyerNickName' => $this->get('security.token_storage')->getToken()->getUser()->getUsername(),
                            'buyerEmail' => $data['contact']['buyerEmail'],
                            'buyerEntity' => $this->get('security.token_storage')->getToken()->getUser()
...
//                                $message="You have successfully contacted ".$bookDeals[0]->getSeller()->getUsername().". Please go to message board to view replies.";
                        $message = "Has contactado con éxito a " . $bookDeals[0]->getSeller()->getUsername() . ". Por favor, vaya al tablero de mensajes para ver las respuestas.";
                        $buyerInfo = array(
                            'buyerNickName' => $this->get('security.token_storage')->getToken()->getUser()->getUsername(),
...
//                                'successTitle'=>"Successfully Contacted ".$bookDeals[0]->getSeller()->getUsername(),
                        'successTitle' => "Contactado correctamente " . $bookDeals[0]->getSeller()->getUsername(),
                    ...
                    return $this->_createJsonResponse('error', array(
//                                'errorTitle'=>"Could not contact",
//                                "errorDescription"=>"Sorry, we could not contact. Please Try again. ",
                        'errorTitle' => "Imposible contactar",
                        "errorDescription" => "Lo siento, imposible contactar, inténtalo de nuevo.",
                        "errorData" => $contactForm), 200);
...
//                            'errorDescription' => "Please Check your already contacted Book list."
                    ...
                    'errorTitle' => "Ha sido contactado previamente",
                    'errorDescription' => "Compruebo tu listado de libros ya contactos."
                ), 400);
            }
